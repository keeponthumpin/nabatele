"""
interfaceextension — NABATELE UI design tokens.

Single source of truth for the prototype's visual language. Colours are
exposed as `Color` dataclass instances (normalized rgba floats, 0..1)
inside the `Colors` dict, keyed by semantic role:

    parent.interface.Colors['topbar_bg'].r        # 0.0706
    parent.interface.Colors['cyan'].rgba          # (0.2118, 0.7725, 0.8392, 1.0)
    op('interface').Colors['amber'].rgb           # (0.902, 0.6353, 0.2353)

Typography, type scale, spacing, radii and icon glyphs follow the same
§7c spec and are exposed as plain dicts / accessors.
...
class Color:
	...
	def with_alpha(self, a):
		"""Return a copy at a new alpha, e.g. Colors['home'].with_alpha(0.5)."""
...
def _hx(hexv, a=1.0):
	"""Shorthand used in the colour table below."""
...
class interfaceextension:
	...
	#  Call: parent.interface.Colors['topbar_bg'].r                       #
	# ------------------------------------------------------------------ #
	Colors = {
		# ---- structural / surfaces -------------------------------------
	...
	# ------------------------------------------------------------------ #
	Fonts = {
		'mono': f'{project.folder}/Resources/Fonts/mono.ttf',
		'sans': 'Inter, system-ui, -apple-system, Segoe UI, sans-serif',
	}

	# Type scale in px, keyed by role.
	Type = {
		'mode_num':      26,
		'insp_empty':    24,
		'insp_title':    20,
	...
	def ToTable(self, dat):
		...
		for name, c in self.Colors.items():
			dat.appendRow([name, c.hex,
						   round(c.r, 4), round(c.g, 4),
						   round(c.b, 4), round(c.a, 4)])
		return dat

	# def onInitTD(self):
	# 	pal = self.ownerComp.op('palette')
	# 	if pal is not None:
	# 		self.ToTable(pal)